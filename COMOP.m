function z = COMOP(x,y,stat1,stat2,Map,Shape,Vrank,Amod,VRes,Vmod,Smod)
%COMOP 此处显示有关此函数的摘要
%   此处显示详细说明
    px=x(1:2,:)+stat1(1:2,:);
    py=y(1:2,:)+stat2(1:2,:);
    ...
    h1(h1<0)=0;
    as=~all(px==stat1(1:2,:),1);%计算我方该回合是否行动（我方状态）
    ts=~all(py==stat2(1:2,:),1);%计算敌方该回合是否行动（敌方状态）
    ...
    z1=sum(AttackTests(d1,h1,as,x(4,:),stat1(3,:),tg1,td1,ts(x(3,:)),Vrank,Amod,VRes,Vmod));
    ...
    z2=sum(AttackTests(d2,h2,ts,y(4,:),stat2(3,:),tg2,td2,as(y(3,:)),Vrank,Amod,VRes,Vmod));
    z=[-z1,z2-5]';
end
